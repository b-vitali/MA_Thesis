...
for this Thesis. 
The short introduction on the Standard Model and some of its extensions 
will justify the interest to pursue experimental searches for Charged Lepton Flavour Violation. 
...
only the most significant aspects of the muon-based experiments will be reviewed.} 

\section{Leptons in the Standard Model}
...
\subsection{Charged Lepton Flavour Violation with $m(\nu)\neq 0$}
The observation of neutrino oscillations has shown that neutrinos 
are not massless particles {\bf{Magari inserire referenza}}. 
There are numerous ways to deal with neutrino mass terms in the theory,  
but lepton flavour violation in the charged current interactions 
is controlled by the Pontecorvo-Maki-Nakagawa-Sakata (PMNS) matrix {\bf{Referenza}}. 
{\em{This matrix diagonalises the neutrino mass matrix in the base of diagonal charged lepton masses 
(creating an explicit link between flavour and mass).}}\\
In this framework, 
...
by loop diagrams which involve neutrinos and the W boson (Fig. \ref{_feynman_SM}).
The estimates of the Branching Ratio for these processes are model dependent
and well beyond the scope of this Thesis: they can be found in the literature  \cite{signorelli}. 
The interesting fact is that in many models CLFV-violating processed are suppressed
by the sum of $(\Delta m_{ij}^2/M_W^2)^2$, where $\Delta m_{ij}^2$ is the mass-squared difference between neutrinos mass eigenstates. 
Since $\Delta m_{ij}^2$ is much smaller than $M_W$, the expected Branching Ratios are very low, 
of the order of $BR(\mu\rightarrow e\gamma)= 10^{-55}\div10^{-54}$ \cite {Petcov}. 
Physics processes with this probabklity are way below the sensitivity of the current  or near-future experiments.
This means that any experimental evidence of CFLV 
would imply some missing piece in the extension of the lepton sector of the Standard Model.\\\\
...
Although there are numerous non trivial extensions of the Standard Model, 
we will limit our discussion only to super-simmetric CLFV.
The first reason for our choice is that SUSY theories played 
a significant role in the speculations on the gauge hierarchy problem 
(why the Higgs has the measured mass, 17 orders of magnitude below the Planck scale). 
The second reason is that the separation between the electroweak scale 
and the neutrino mass scale plays a significant role in the suppression of CLFV. 
This is not the case any more if one inserts SUSY partners in the loops for CLFV-violating processes, 
({\bf{Mi domando che cosa significa}} which mass scale is defined by a SUSY breaking scale $m_{SUSY}$).\\
Generally speaking, in SUSY models lepton and slepton matrices are not aligned. 
In other terms, physical sleptons end up being mixtures of different flavours. 
In this situation, CLFV arises from the interaction of Standard Model leptons, 
their SUSY partners (sleptons and sneutrinos), 
and potential mixture of other SUSY and Standard Model particles. 
In order to asses the rates for CLFV processes in SUSY models, 
it is necessary to know the masses of the particles appearing in the diagrams 
and the composition in flavour of the sleptons.
There are numerous flavour structures models: 
some are trivial (aligned leptons and sleptons mass matrices), 
some are controlled by some flavour symmetry (such as SU(3), U(2), \ldots) 
and other predict anarchical structures. 
An honourable mention in this field of speculation is the SO(10) SUSY Grand Unified Model (GUT).

\subsection{SUSY seesaw}
In this Section we will briefly describe the simplest mechanism that allows to include
a neutrino mass term in the theory. 
In the Standard Model this can be done using 
a conjugate Higgs doublet introducing a Dirac mass term
...
that the solution should be searched for in an alernative way.
A different term that would satisfy the local gauge invariance would be 
a Majorana mass term (formed by a $\nu_L$ and a $\bar{\nu}_R$, 
which trasforms as a singlet under the SM gauge transformations):
$$\mathcal{L}_M=-\frac{1}{2} M(\overline{\nu}_R^c\nu_L+\overline{\nu}_L\nu_R^c)$$
In this case, a coupling between particle and antiparticle is present, 
...
involving muons has been proposed by de Gouv\^{e}a \cite{deGouvea}:
\begin{align}
\mathcal{L}_{CLFV}=
...
\end{align}
In this expression, 
$m_\mu$ is the muon mass, $F^{\mu\nu}$ the photon field, $L$ and $R$ indicate the chirality of the fermion field.
On an intuitive level the two terms correspond to 'dipole' and 'contact' 4-fermion interaction.
There are two independent parameters,  $\Lambda$, the mass scale, 
and $\kappa$, which weights the two terms. 
Fig. \ref{_deGouvea} shows the relationship between the Branching Ratio 
of $\mu\rightarrow e\gamma$, $\mu\rightarrow eee$ and $\mu N \rightarrow e B$
as a function of the parameter $\kappa$. The parameter space for muon CLFV
that has been excluded by previous experiments and the region that future
experiments will be able to probe are also reported. The upper limits expected
at 90\% CL for the future experiments MEG upgrade, Mu3e, Mu2e and Mu2e at PIP II 
(Proton Improvement Plan-II) are also shown.

\begin{figure}[h!]
\includegraphics[scale=0.8]{deGouvea}
\caption{Current and future limits expected on $\mu\rightarrow e\gamma$,
$\mu\rightarrow eee$ and $\mu N \rightarrow e B$ conversion in terms 
...
\noindent A general conclusion on this (absolutely non exhaustive) introduction 
to SUSY could simply be that 
there are numerous fascinating models but, even within a specific model, 
there are no 'guaranteed' minimum rates to be expected. 
Due to the flexibility of these models, 
rates can be vanishing or exceed the current limits.
...
and the comparison between this and the one by Hungerford is shown in Fig. \ref{_Comparison} \cite{io:comparison}.\\
...
The relation between the generated and reconstructed spectra is shown in fig. \ref{_deuteron_preco-vs-pgen_trk0}.
In fig. \ref{deuteron_preco-vs-pgen_trk0
} we find the distribution of the reconstructed momentum as a function of the generated value. The spectrum at generation has already been shown in fig. \ref{}. 
\begin{figure}[!htb]
\centering
...
This was expected because, as already discussed, the Hungerford parameterization for these particles was done simply assuming the same kinetic energy distribution.
As a result the values to take in consideration are the one obtained with the current parameterization.\\
...
\hline
\hline
Protons& \makecell{2.65(4) \\ 1.99(4)} & \makecell{ 2.69(4) \\ 1.95(4)} \\
\hline
Deuterons & \makecell{0.54(2) \\ 0.43(2)} & \makecell{ 0.99(3) \\ 0.80(3)} \\
\hline
\end{tabular}
...
\noindent In a full Mu2e event the steps required for the reconstruction are the same but for an obvious major difference: not all the hits in the event are made by the particle we are interested.
At this point the only thing we are missing is a first skim of the StrawHits, to reduce the background hits.
The time clustering of course helps us separating possible interesting hits from the rest but two additional step can aid us before even starting the reconstruction: the energy deposit in the straw and the number of hits associated to a TimeCluster.
...
The presence of a possible bug is under study but for our purpose we have to roll back and use a slightly older version of the code with the MDC2018 dataset. The proton dataset which was used in the mixing procedure was generated using the Hungerford.

\section{SH energy}
Hits made by different particles with different $\beta$ are associated to different values of the total energy deposited. 
Although confusing, often the Mu2e jargon this is called \textit{charge} and is measured in MeV.
