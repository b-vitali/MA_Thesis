...
\title{In situ monitoring\\ of the stopped muon flux at Mu2e\\\textbf{V\_0.8.1}}
...
The Mu2e experiment at Frmilab is one of the pioneering experiments in the search for Charge Lepton Flavor Violation, looking in particular to the neutrino-less coherent conversion of a negative muon in the field of an aluminum nucleus: the signal is a monoenergetic conversion-electron of $E_{CE} \approx 104.97$ MeV and the measured value will be $R_{\mu e}= (\mu^-N \rightarrow e^-N)/(\mu^-N \rightarrow \text{all } \mu \text{ captures})$ \cite{MTDR}.
This process is forbidden in the Standard Model but allowed in many of its extensions: with minimal changes to include neutrino masses and oscillation, the BR for processes like this one or the forbidden $\mu\rightarrow e \gamma$ are expected to be $\mathcal{O}(10^{-54})$.
Values like this are clearly below any currently achievable experimental sensitivity, meaning that any measurement would be signal of new physics.
The limit for the conversion was set by SINDRUM II at $7\times10^{-3}$ \cite{SINDRUMII} and the aim of the Mu2e collaboration is an improvement of 4 order of magnitudes, down to SES $\approx 3\times 10^{-17}$.\\
The Mu2e experiment can be conceptually divided in three stages: interaction of the primary proton pulse with the target and the production of $\pi$ and $\mu$; collection and transport down to the aluminum stopping target; interaction with the stopping target and measurements of the output particles.
The measurement of the stopped muon flux is of cardinal importance in measuring the $R_{\mu e}$.
The baseline design includes two detectors (HPGe and LaBr3) developed to measure this flux, counting $\gamma$ emitted by the muonic atoms.
The studies performed indicate that these two systems can be used reliably in the evaluation of the denominator of $R_{\mu e}$.\\
The number of stopped muons is clearly proportional to the number of proton on target, which itself is dependent on the extraction system for the proton beam. 
The beamline to Mu2e will use a resonant extraction to create the proton pulses and this method is characterized by intensity fluctuations on the time scale of milliseconds. 
These fluctuations, aside from the proportional fluctuation in the number of muons, have a non trivial impact on the overall performance of the apparatus. 
For example higher intensity proton bunches translate to higher veto rate by the Cosmic Rays Veto system, which is triggered by secondary particles of the proton interaction. 
Another example is the effect of the instant luminosity on the reconstruction efficiency: higher intensity translate to higher occupancy, which in turn reduces the reconstruction efficiency.
As of today, no system allows to monitor these fluctuations. The aim of this work is to fill this gap.\\

\newpage
\noindent The method here developed to monitor the fluctuations of the stopped muon flux, and by extension the fluctuation in the proton pulses intensity, relies on the reconstruction of proton tracks in the straw-tube tracker.
One of the possible processes a stopped muon can undergo is the nuclear capture and, in a fraction of cases, charged particles can be ejected as result. 
The protons present in the tracker are thus coming from the stopping target and proportional to the number of stopped muons.\\
The reconstruction algorithms were developed by the Mu2e collaboration with the explicit (and sometimes implicit) aim to reconstruct tracks of electrons in an energy range around the hypothetical conversion electron. 
The protons we are interested in behave quite differently from these electrons and the cardinal task of this study was to tailor the reconstruction routine to these particles. This has been done analyzing simulated event consisting of a single proton, optimizing the procedure and understanding the features of these tracks.\\
The usage of a pulsed beam and the need to allow produced $\pi$ to decay and $\mu$ to interact in the stopping target force the definition of a Mu2e \textit{event}: an event comprised by everything which happens in a $1.7\ \mu$s window following the incidence of the proton pulse.
This means that the reconstruction will be performed in an environment crowded by many different particles.\\
The next step in this study was hence to evaluate the performance of the method in a fully fledged Mu2e event and what kind of selections would improve it.\\
The results obtained are satisfactory (few protons reconstructed per event), indicating that a monitoring on the timescale of milliseconds is possible, with a statistical uncertainty of less than $5\%$.
...
employed for proton tracks reconstruction.  
...
using the simplest possible Monte Carlo sample with only one particle simulated per event.}
...
To estimate the method performance through Monte Carlo simulation,
we have first to make sure we have a clear understanding of the 
spectra of the particles generated after nuclear capture.\\
A theoretical model has been developed by Lifshitz and Singer \cite{Lifshitz} 
to explain the observed emission of charged particles following muon nuclear capture 
but there is still a lack of experimental data, particularly regarding Al nuclei.
Due to the lack of data, the Mu2e Collaboration has used  the experimental spectrum 
of charged particles generated from muon captures on Si 
measured by Sobottka and Wills (Fig. \ref{_sobottka}, \cite{Sobottka}).
This spectrum was adapted to aluminum by Hungerford \cite{Hungerford}.
Since no experimental data are available for deuterons either, 
Mu2e decided to use the same kinetic energy spectrum as for protons.

\begin{figure}
\centering
\includegraphics[scale=0.8]{sobottka}
\caption{Measured spectrum of protons emitted following muon nuclear capture in Si \cite{Sobottka}. 
This spectrum is obtained by subtracting from the data the muon-decay electron background. 
A dip at $\approx1.4$ MeV is clearly visibile.}
\label{_sobottka}
\end{figure}

\noindent 
The AlCap \cite{AlCap:2018} and TWIST \cite{TWIST:2020} Collaborations 
have been leading the ongoing efforts dedicated to measure the spectra 
...
An alternative parameterization that takes into account the new results 
has been recently presented \cite{Pasha:spectra} and the comparison between 
the news one and the previous one by Hungerford is shown in Fig. \ref{_comparison2} \cite{io:comparison}.

\noindent The proton spectrum obtained by Sobottka (Fig. \ref{_sobottka}) 
is the result of subtracting the muon-decay electron background from the measurements. 
The presence of a dip at low non-zero kinetic energy ($\approx 1.4$ MeV) of the spectrum, 
and the related uncertainties have been the subject of a lot of  discussion 
within the Mu2e Collaboration \cite{io:sobottka} 
when trying to develop a possible parameterization. 
The left side of the spectrum ($E_k\lessapprox1.4$ MeV) is identified as Al$^{27}$ recoil from neutron emission. 
...
These results support the choice of parameterization proposed this year 
...
\hline
\end{tabular}
\caption{Current values for the ejection of particles from muon nuclear capture 
from TWIST \cite{TWIST:2020} and AlCap \cite{AlCap:2020} Collaborations
{\bf{Mi domando che cosa sono le quantit\'a misurate}}.}
\label{T_AlCap_TWIST}
\end{table}
...
\caption{Recent measurements of proton (Left) and deuteron (Right) spectra performed
...
\caption{Comparison \cite{io:comparison} between the Hungerford parameterization \cite{Hungerford} and the parameterization proposed by Murat \cite{Pasha:spectra} for proton (red) and deuteron (blue). 
...
\noindent 
We do not have a satisfactory understanding of the global picture yet because, 
on top of the unknowns related to the low momentum region of the spectra, 
we also lack information for the high momentum region ($p\gtrsim200$ MeV/c, $E_k\gtrsim 20$ MeV).

\noindent
Furthermore, the presence of the central hole in the tracker 
and the proton absorber that surrounds the stopping target introduce an additional layer of complexity. 
The presence of the hole in the detector implies that particles with transverse momentum below $\sim70$ MeV$/c$ 
are outside the tracker acceptance and thus do not generate hits. 
This determines the region of the ejected particle spectrum Mu2e is sensitive to. 
The proton absorber has the function of reducing the overall particles flux 
and thus detectors occupancy, but at the cost of degrading the momentum 
of the remaining particles. 
The consequence is that determining the spectra of particles that can be observed in the tracker is pretty hard.\\ 

\noindent 
The considerations made so far show that using protons generated in the muon capture 
as a normalization for the conversion electron search would result in large systematic uncertainties 
and consequently reduce the Mu2e Single Event Sensitivity. 
Nonetheless, this technique is still extremely precious to monitor the fluctuations in time of the proton on target, 
proportional to the stopped muon flux, which is something that as of today no other Mu2e detector allows. 
The systems already under development (based on HPGe and LaBr3) have a complementary function: 
they can be reliably used to determine the total normalization and the average POT, 
but do not allow to monitor on the time scale the beam intensity is expected to fluctuate.\\
This method is powerful because of the large total number of expected protons per event ($1.7\ \mu$s). 
We can perform a back-of-the-envelope calculation to provide a preliminary estimate of the proton yield:
\begin{center}
\begin{footnotesize}
number of POT $\times$ $\mu_{stopped}$/POT $\times$ P($\mu$ nuclear capture) $\times$ P(proton ejection) $\sim 1.5\times10^3$ p/event
\end{footnotesize}
\end{center}
The proton yield is determined using the most recent estimates: 
$3.9\times10^7$ for the number of POT (protons on target); 
$1.6\times10^{-3}$ stopped muon per POT; 
$0.61$ for the probability of muon nuclear capture; 
$0.045$ is the probability of proton ejection.\\
What may help us in reconstructing these particles is that protons from nuclear captures 
have specific characteristics: they are highly ionizing and have opposite charge 
...
from the other backgrounds and somewhat unique.
These characteristics will be fundamental to identify them.

\section{"Single proton" Monte Carlo events}
The first step in developing the tools for our simulation has been the study single particle events. 
This has been done in two steps: 
the first one has been using particles generated with a flat momentum distribution
to debug the procedure; 
the second one has been generating particles with the expected spectrum 
to determine the algorithm performance.

\subsection{Flat proton momentum distribution}
The reason for starting from a flat distribution is to study track reconstruction on its own, 
...
with a flat momentum distribution in the range $p\in[100,600]$ MeV$/c$ in the stopping target. 
The particles stored in the output file are only those which interact either with the actual Mu2e detectors, 
or with some "virtual detectors" that can be positioned in the simulation 
of the Detector Solenoid for debugging purposes.\\
The distribution of the generated momenta for particles interacting 
at least once in the tracker is shown in Fig. \ref{_flat_Lambda_p-gen1SH}. 
The plot justifies the choice of the low end of the momentum range of the generated particles: 
no proton under 100 MeV$/c$ interacts with the tracker due to the presence 
of the absorber surrounding the stopping target. 
The plot also provides the simplest possible definition 
of the tracker geometrical acceptance. \\

\begin{figure}[h!]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/flat/Lambda_p-gen1SH}
\caption{Momentum distribution of particles which interact 
at least one time with the tracker and generate at least one StrawHit. 
...
The consequence is that the absorber contributes to reduce the occupancy of the detector 
and also determines the minimum energy particles should have to reach the tracker. 
To clarify this point, 
we have simulated 100k protons both with and without the absorber (Fig. \ref{_proton_absorber}). 
The right plot shows that the presence of the proton absorber reduces the number 
and increases the momentum of particles that reach the tracker. 
...
\caption{Distribution of the number of StrawHits generated in the tracker by single particles 
as a function of the particle momentum, with no proton absorber (Left plot)
and with proton absorber (Right plot).}
...
of the generated particle momentum.
...
but we can anticipate it is due to the simultaneous presence of the hole in the tracker 
and the momentum range of the particles we are trying to identify.

\begin{figure}[h!]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/flat/all_SHvsGenp}
\caption{Number of StrawHits reconstructed per event as a function of the generated particle momentum.}
\label{_all_SHvsGenp}
\end{figure}

\noindent The first step of track reconstruction is grouping the hits 
that might have been generated by the same particle. 
As we have explained in Chapter 3, 
track reconstruction algorithms exploit both hit position and time
to determine the TimeClusters. 
The way the code was developed introduces a bias towards patterns generated by 
$105$ MeV$/c$ electrons 
and this makes the code less capable of reconstructing the TimeClusters generated by protons. 
Before turning off the MVA used by default to search for TimeClusters, 
we found no hits associated to the same time peak. 
This is because the MVA was trained to identify patterns due to conversion electrons. 
After deactivating the MVA, 
the StrawHits are now grouped in TimeClusters and Fig. \ref{_TimeClusterPar_nSH} 
shows the distribution of the number of StrawHits per TimeCluster. 
The basic element of the reconstruction are not StrawHits but ComboHits 
and Fig. \ref{_flat_TimeClusterMVA_nCH-vs-p} show how the number 
of these contained in a TimeCluster is related the momentum 
of the generated particle: 
the non trivial shape is clearly related to the shape visible in Fig. \ref{_all_SHvsGenp}. \\
...
\caption{Number of StrawHits associated to the TimeCluster reconstructed in each event, 
as a function of the generated particle momentum.}
...
\caption{Number of ComboHits associated to the TimeCluster found in each event 
as a function of the generated particle momentum.}
...
Fig. \ref{_TimeClusterMVA_ntcl-vs-pgen}
shows that for low momentum particles we often end up with more than 1 TimeCluster per event
even in single particle events.\\
...
\begin{figure}[!htb]
    ...
    \begin{minipage}{.49\textwidth}
		...
		\caption{Number of reconstructed TimeClusters associated 
		as a function of the generated particle momentum. 
    ...
    \begin{minipage}{0.49\textwidth}
		...
		\caption{Number TimeClusters associated to each event as a function of the generated particle momentum. 
		This is obtained after the implementation of the function to merge TimeClusters.}
		\label{_TimeClusterPar_ntcl-vs-pgen}
	\end{minipage}
\end{figure}

\noindent If one checks the events with more than one TimeCluster 
and takes into account that these correspond to single proton events, 
it is possible evaluate under what assumption it may be reasonable to merge them. 
The distribution of the angular distance $\Delta \phi$ in the $XY$ plane 
and the time distance $\Delta t$ between two TimeClusters found in the same single-particle event
are shown in Fig. \ref{_TimeClusterPar_ProtonPeaks}. 
This distribution can be used to decide the distance range in which 
we require TimeClusters to be merged to assign 
the hits to the particle candidate in a more reliable manner. 
The merging is performed in the early stage of the TimeCluster search 
and the following steps remove and add the hits if the merging 
results in a TimeCluster not meeting the requirements.
There seems to be two components to this plot: clusters with high $\Delta t$ with low $\Delta \phi$ and vice versa.

\begin{figure}[h!]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/flat/TimeClusterPar_ProtonPeaks}
\caption{Distribution of the time and angular distance in the transverse plane 
between the TimeClusters found in the same single particle event. 
This distribution looks quite sporadic, 
but clearly there are two contributions: 
high $\Delta t$ with low $\Delta \phi$ and vice versa.}
\label{_TimeClusterPar_ProtonPeaks}
\end{figure}

\noindent After merging the hits collection, 
the distribution of the number of TimeClusters per event changes in the desired direction: 
Fig. \ref{_nTC_comparison} shows that the number of events with more than 1 TimeCluster 
is drastically reduced, while Fig. \ref{_TimeClusterPar_ntcl-vs-pgen} shows in doing so we did not distort the overall distribution.
We could reduce the contribution due to these cases by changing the requirements 
but the trade would be the risk of merging non related TimeClusters 
when multiple particles will be present in the simulation.\\

\begin{figure}[h!]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/flat/nTC_comparison}
\caption{The introduction of a function to merge TimeClusters close in space an time reduces 
the probability of finding more than 1 TimeCluster per event. 
The red distribution is after the change.}
\label{_nTC_comparison}
\end{figure}

\paragraph{Bore.} 
The presence of the hole and the finite size of the tracker have a significant impact 
on most distributions related to particle reconstruction.
A good way to understand this mechanism is to look in which plane of the tracker 
the StrawHits are generated; 
the planes where the first and last StrawHits are located are particularly interesting. 
Fig. \ref{_Lambda_first-z} and \ref{_Lambda_last-z} show these distributions
as a function of the generated particle momentum.\\
Low momentum particles tend to leave the first hit at the entrance of the tracker. 
As momentum is increased ($p \in [150,250]$ MeV$/c$), 
particles can arrive 
at the front of the tracker after having completed  a full rotation. 
The result is that particles travel partially inside the hole, 
moving downstream the first StrawHit.
For even higher momentum, 
particles do not complete a full rotation before reaching 
the tracker and interact with the first stations.
A similar behaviour is present also in one checks the last hit in the tracker. 
...
\caption{Station of the tracker where the first hit associated to the track is located 
...
\caption{Station of the tracker where the last hit associated to the track is located 
as a function of the particle momentum.}
...
\caption{Sketch of a particle produced in the stopping target and interacting only 
at the end of the tracker because it travels in the hole due to the pitch of the helix.}
\label{_blender}
\end{figure}

\paragraph{Pitch.}
The next step in the process of track reconstruction is searching for an helix compatible 
with the StrawHits which compose a TimeCluster.
The result of this search clearly depends on both the quality of the reconstructed TimeCluster 
and the way the helix finder is implemented (the algorithm was discussed in Chapter 3). 
We can start by checking the dependence of the estimated angular velocity 
$\mathrm{d}\phi/\mathrm{d}z = 1/ \lambda$ on the generated momentum. 
This dependence, shown in Fig. \ref{_TimeClusterMVA_Lambda-vs-p}, 
seems to present a sharp cut for the maximum value of lambda.  
Some requirements in the module used to find the helix are set 
by default to improve the reconstruction of helices generated by particles 
with momentum $\sim 105$ MeV$/c$. 
If these requirements are relaxed, 
the dependence changes as shown in Fig. \ref{_Lambda_Lambda-vs-p}. 
Now, the dependence seems to be correct: 
given that the tracker has a maximum radius, 
higher momentum must translate in longer helix pitch.\\

\begin{figure}[h!]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/flat/TimeClusterPar_Helix_lambda-vs-pgen}
\caption{Angular velocity estimated for the TimeCluster as a function 
of the generated particle momentum. 
...
\caption{Angular velocity estimated for the TimeCluster obtained as a function 
of the generated particle momentum. 
...
this change of the requirements improves also the association of the hits to the trajectory. 
This is shown in Fig. \ref{_active_SH_fraction}. 
The reason is quite simple: 
if the parameter $\lambda$ is evaluated incorrectly, the hits end up more distant for me track and are dropped. 
This is particularly true for tracks resembling the example shown in Fig. \ref{_blender}, 
with hits clusters distant from one another. \\
...
\caption{Fraction of StrawHits associated with a helix as a function of the generated particle momentum. 
The plots on the left and right are respectively before and after having relaxed 
the requirements on the algorithm to find the helices.}
\label{_active_SH_fraction}
\end{figure}

\noindent Using $\lambda$ we can estimate the number of rotations the particle 
would complete while traveling the whole length of the tracker. 
The reason for doing this is that visualizing the trajectory is simpler in this way. 
The number of rotations for the reconstructed helices, 
after the requirements for $\lambda$ have been relaxed, is shown in Fig. \ref{_Lambda_Rot-vs-p}. 
This plot shows that low momentum protons roughly perform a full rotation when traveling in the tracker 
while for higher momentum the number of rotations is lower than one. 
The reason of course is that these particles need to be generated at a more shallow angle in order not to interact 
with the external structure of the tracker or the solenoid. The result is a larger pitch and a lower number of rotations.\\


\begin{figure}[h!]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/flat/Lambda_Rot-vs-p}
\caption{Number of rotations the particle would complete while traveling the length of the tracker 
as evaluated from the $\lambda$ obtained from the helix fit. 
This distribution is obtained after relaxing the requirements in the procedure for the helix search.}
...
The first thing to check is how well the reconstruction estimates the particle momentum, 
to see if there are any glaring mistakes. 
The reconstructed momentum versus the generated momentum
is shown in Fig. \ref{_Lambda_preco-vs-pgen}: 
...
The plot shows the expected behaviour, but for low momentum particles, 
...
\caption{Reconstructed momentum (after the Kalman filter) as a function of the generated momentum.}
...
on the reconstructed tracks and how to estimate the normalization. 
...
the denominator must account only for the particles that we could have reconstructed, 
i. e. particles interacting a minimum number of times in the tracker. 
At this point, the only requirement is the presence of at least 5 hits in the tracker, 
justifying the fitting procedure. 
We can then use as normalization the distribution of the generated momentum for particles 
which interacted more than 5 times. 
...
\caption{Reconstruction efficiency for particles with at least 5 hits in the tracker. 
The numerator is the spectrum of the momenta of all the reconstructed particles 
while the denominator is the spectrum of the generated momenta of all particles 
interacting at least 5 times with the tracker.}
\label{_Lambda_eff_trk0-5hits}
\end{figure}

\noindent The distribution shown in Fig. \ref{_Lambda_eff_trk0-5hits} shows interesting structures. 
At low momenta the efficiency is reduced because particles simply do not reach the tracker 
due to the presence of the proton absorber (Fig. \ref{_proton_absorber}). 
...
we saw that is the range where the track topology changes: 
...
the reconstruction is performing at an efficiency $\sim 0.8$ 
which is a remarkable result considering we are adapting an existing fit routine, 
tailored to a different particle and momentum range.\\

\noindent
To asses how the whole procedure, Kalman filter included, has performed,
we can evaluate the $\chi^2/\textrm{d.o.f.}$ and check its correlation 
with the generated momentum. 
The distribution is shown in Fig. \ref{_Lambda_chi2d_trk0}. 
What  is clear from this plot is that, as we expected, 
higher momentum particles follow a trajectory which deviates 
from an helix resulting in low values of the $\chi^2$. 
Another interesting feature of the reconstructed tracks is the number of hits 
that after all the algorithms are run, are associated with a track. 
...
Again, a dip is present around $\sim 250$ MeV$/c$ and for larger momenta 
the number of hits associated to the tracks is even larger.\\
Given these distributions, it seems reasonable to define some minimal cuts 
to asses a 'good' track: $\chi^2/\textrm{d.o.f.}<5$ and $nSH>20$.
With these requirements, the efficiency changes to \ref{_Lambda_eff_trk21-20hits}.
Clearly, the second requirement does not play a major role here but it means that in the mixed events 
we might be able to improve the reconstruction by requiring the minimum number of hits above 5, 
the number strictly needed to perform the fit.

\begin{figure}[!htb]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/flat/Lambda_chi2d_trk0}
\caption{Distribution of the $\chi^2/\textrm{d.o.f.}$ associated to the reconstructed track 
as function of the generated momentum.}
...
\caption{Distribution of the number of active StrawHits associated to the reconstructed tracks 
...
\caption{Efficiency evaluated by requiring $\chi^2/\textrm{d.o.f.}<5$ and $nSH>20$. 
In this instance the denominator has been changed to include only the generated particle momenta 
which left at least 20 SH.}
\label{_Lambda_eff_trk21-20hits}
\end{figure}

\subsection{Ejected spectrum}
The step of using the expected ejected spectrum does not change the basic principles 
and tweaks performed in the reconstruction but provides us some information on what 
we actually expect to see when reconstructing a full Mu2e event. 
For this purpose, $1.5\times10^6$ protons were generated following 
the most recent parameterization for the ejection spectrum.\\
The distribution of the reconstructed versus generated momentum is shown in Fig \ref{_proton_preco-vs-pgen_trk0}. 
As expected from the efficiency estimated in the previous Section (Fig. \ref{_Lambda_eff_trk0-5hits}), 
we do not reconstruct any proton generated with momentum below $\sim 150$ MeV$/c$; 
at the same time the spectrum decreases steeply and the probability to have 
a proton with momentum $p > 250$ MeV$/c$ is low.

\begin{figure}[!htb]
\centering
\includegraphics[width =0.8\textwidth, keepaspectratio]{/plots/ejected/proton_preco-vs-pgen_trk0}
\caption{Reconstructed momentum as a function of the generated momentum 
using the expected spectra for ejected protons.}
\label{_proton_preco-vs-pgen_trk0}
\end{figure}

\paragraph{Efficiency.} We can now estimate the proton reconstruction efficiency, 
using as numerator the distribution of the generated momenta for the particles reconstructed 
and as denominator the ejection spectrum scaled to the number of generated particles. 
...
\section{Deuterons and systematic effects}
Until this point we only discussed single proton events. 
From table \ref{T_AlCap_TWIST} we know deuterons are roughly half the protons in number 
and their distribution is slightly different.
Additionally we assumed the parameterization of the spectrum to be correct. 
These two points need to be addressed before moving to mix-events.

\subsection{Deuterons}
On a procedure level, 
the fit for a deuteron track is the same as for a proton because the key variables are 
the momentum and the sign of the charge.
It should be clear that if we apply this reconstruction to a full simulated Mu2e event 
we are going to reconstruct both particles and we need to assess the reconstruction efficiency for deuterons. 
Fig. \ref{_comparison} shows the overall comparison of the various spectra  
(Fig. \ref{_comparison2} shows the detail at low energy of this plot).\\
Following what has been done in the last step for protons, 
the reconstruction has been run on $1 \times 10^6$ deuterons generated with the expected spectrum. 
Fig. \ref{_deuteron_preco-vs-pgen_trk0} shows the distribution of the reconstructed momentum 
as a function of the generated momentum. 
The spectrum at generation level has already been shown in Fig. \ref{_comparison}. 
...
the parameterization of the spectra for both protons and deuterons is still under development. 
...
Given the fact that a consensus has not been reached on the parameterization yet, 
it might a good idea to see how the reconstruction for both particles would change using Hungerford's spectrum.\\
What should be underlined is that while both parameterizations have some validity for protons, 
the Hungerford spectrum for deuterons is based on the assumption that the kinetic distribution might be the same 
as the one for protons. 
This is why we expected a greater discrepancy for deuterons. 
Table \ref{T_syst} reports the results for the same reconstruction procedure run on all the data-sets. 
The values obtained for protons are fully compatible meaning that, 
even though the distributions are different, 
with the statistics run the track-counting is not sensitive to any systematic due to the different spectra.
...
{\itshape 
The performance of the method developed in the previous Chapter 
has been investigated by studying 'full' Monte Carlo events (called `mix-events') 
where
all the expected backgrounds of the experiment have been included.
Now the challenge is implementing a robust way of pre-filtering 
the StrawHits generated by other particles (primarily electrons). 
This can be done by exploiting the energy deposit 
and tightening the number of required hits
in light of the distribution reported in Fig. \ref{_Lambda_nSH_trk0} 
of the number of hits associated to the reconstructed track.}

\paragraph{First, a caveat.} 
At the time of writing this Thesis, 
the most up-to-date Monte Carlo dataset (MDC2020) was still under development 
and could not be used to validate the reconstruction tailored. 
The most recent available mixed dataset (simulated events with the full background) 
was part of MDC2018, but the current version of the offline code 
had lost its backward compatibility due to a number changes, 
like the way the tracker readout electronics was simulated 
and how the digitization of the signals  was created. 
In the attempt of validating the analysis modules before the new dataset was ready, 
...
Unfortunately, as the result of this procedure, 
...
we have to roll back and use a slightly older version of the code 
with the MDC2018 dataset. \\
These files had been generated before the new measurements 
from AlCap and TWIST were made available,
and as result the particles were generated following the Hungerford spectra. 
As discussed in the previous Chapter, this has impacted both the momentum distributions 
